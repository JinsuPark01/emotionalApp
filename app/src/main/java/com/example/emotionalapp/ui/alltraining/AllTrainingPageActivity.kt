...
import com.example.emotionalapp.databinding.ActivityAllTrainingPageBinding
import com.example.emotionalapp.ui.open.BottomNavActivity

class AllTrainingPageActivity : BottomNavActivity() {

    // 뷰 바인딩으로 전환
    private lateinit var binding: ActivityAllTrainingPageBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // 뷰 바인딩 설정
        binding = ActivityAllTrainingPageBinding.inflate(layoutInflater)
    ...
    private fun loadTrainingData() {
        ...
        val sampleData = listOf(
            ...
            TrainingItem("et001", "1주차 - 정서인식 훈련", "나의 감정을 정확히 알아차리기", TrainingType.EMOTION_TRAINING, "100%", R.color.button_color_emotion, EmotionTrainingActivity::class.java),
            TrainingItem("bt001", "2주차 - 신체자각 훈련", "몸이 보내는 신호에 귀 기울이기", TrainingType.BODY_TRAINING, "잠김", R.color.button_color_body, BodyTrainingActivity::class.java),
            TrainingItem("mwt001", "3주차 - 인지재구성 훈련", "생각의 틀을 바꾸는 연습", TrainingType.MIND_WATCHING_TRAINING, "잠김", R.color.button_color_mind, MindWatchingActivity::class.java),
            // --- 여기가 핵심 수정 부분입니다 ---
            TrainingItem("eat001", "4주차 - 정서표현 및 행동 훈련", "건강하게 감정을 표현하고 행동하기", TrainingType.EXPRESSION_ACTION_TRAINING, "잠김", R.color.button_color_expression, ExpressionActionTrainingActivity::class.java)
        )
