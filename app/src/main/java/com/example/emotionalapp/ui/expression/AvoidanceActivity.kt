...
// View.OnClickListener를 더 이상 사용하지 않습니다.
class AvoidanceActivity : AppCompatActivity() {
    ...
    private fun setupClickListeners() {
        ...
        binding.btnTopic1.setOnClickListener {
            navigateToDetail("정서 회피란?", R.string.topic_content_what_is_avoidance)
        }

        binding.btnTopic2.setOnClickListener {
            navigateToDetail("회피해도 괜찮은 정서 vs 회피하면 안 되는 정서", R.string.topic_content_emotions_to_avoid)
        }

        binding.btnTopic3.setOnClickListener {
            navigateToDetail("정서 회피의 단기적 효과", R.string.topic_content_short_term_effects)
        }

        binding.btnTopic4.setOnClickListener {
            navigateToDetail("정서 회피의 장기적 결과", R.string.topic_content_long_term_effects)
        }

        binding.btnTopic6.setOnClickListener {
            // '퀴즈' 버튼은 QuizActivity를 직접 실행합니다.
            val intent = Intent(this, EmotionAvoidanceQuizActivity::class.java)
